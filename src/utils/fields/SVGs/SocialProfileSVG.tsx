...
import { GeneralInputSVG } from "./GeneralInputSVG";

export const SocialProfileSVG = (props?: any) => {
  const handleChange = () => {};
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [clickedId, setClickedId] = useState(""); // State to store clicked id

  const editSocialHandle = (e: any) => {
    e.preventDefault();
    ...
    <>
      <div className="mt-6.5">
        ...
        <div className="flex items-center justify-center gap-3.5">
          <a
            ...
            onClick={editSocialHandle}
            id="Enter facebook handle"
          >
            <svg
      ...
      >
        <div className="text-center">
          ...
          <button
            onClick={closeModal}
            className="mt-6 px-4 py-2 bg-blue-500 text-white rounded border border-primary bg-primary hover:bg-blue-700 focus:outline-none focus:ring focus:border-blue-300"
          >
            Update Details
          </button>
          {/* Add any additional modal content as needed */}
        </div>
      </GeneralModal>
    </>
  );
