...
import { maxFileSize } from '../../consts/const';
    ...
    const handleUpload = async (file: any) => {
        try {
            ...
            if(file.size > maxFileSize){
                let errorMessage=`File size beyond max of 1mb. Please use a smaller size`
            ...
            const url = `${http.setURL}user/${auth.getCurrentUser().uuid}/profile`
            const headers = http.setJwtHeaders()
            headers['headers']['Content-Type'] = 'multipart/form-data'
           
           

            const { data } = await http.put(url, formData, headers)
            if (data.status === true) {
                errorMessage
